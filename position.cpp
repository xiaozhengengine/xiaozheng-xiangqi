...
Bitboard Position::blockers_for_king(Bitboard sliders, Square s, Bitboard& pinners) const {
  ...
  while (snipers)
  {
    ...
    Bitboard  b;
    if(isCannon){
        b = between_bb(s, sniperSq) &  (pieces() ^ sniperSq);
    }else{
        b = between_bb(s, sniperSq) & (occupancy);
    }
    ...
    while (attackers)
    {
        ...
        while (candidates)
        {
            ...
            if (chase_legal(make_move(from, to), checkUs))
                chase |= chase |make_chase(idBoard[to], idBoard[from]);
        }
    ...
    for (int i = 4; i <= st->pliesFromNull; i += 2)
    {
        ...
        if (stp->key == st->key)
        {
            ...
            for (int j = 4; j <= i; j += 2)
            {
                ...
                if (stp->key == st->key)
                {
                    if( (chaseThem || chaseUs)){
                        result =(!chaseUs ? mate_in(ply) : !chaseThem ? mated_in(ply) : VALUE_DRAW);
                    }else{
                        result=VALUE_DRAW;
                    }
