    ...
    while (bb)
    {
        ...
        if (Pt != CANNON){
            if(Pt != PAWN){
                ...
                b =pawn_attacks_bb(Us, from) & target;
            }
        ...
        if (Type == QUIET_CHECKS){
            if(Pt == CANNON){
                b &= ~line_bb(from, pos.square<KING>(~Us)) & pos.check_squares(Pt);
            }else{
...
ExtMove* generate(const Position& pos, ExtMove* moveList) {
  ...
  if(us == WHITE){
      return generate_all<WHITE, Type>(pos, moveList);
  }else{
      return generate_all<BLACK, Type>(pos, moveList);
  }
...
ExtMove* generate<EVASIONS>(const Position& pos, ExtMove* moveList) {
    ...
    if(us == WHITE){
        return generate_moves<WHITE, EVASIONS>(pos, moveList, target);
    }else{
        return generate_moves<BLACK, EVASIONS>(pos, moveList, target);
    }
