...
//如果系统不支持64位系统，需要自定义Bitboard结构体并重载操作符
...
enum Value : int {
  ...
  RookValueMg    = 1362,  RookValueEg    = 1468,
  AdvisorValueMg = 230 ,  AdvisorValueEg = 150 ,
  CannonValueMg  = 555 ,  CannonValueEg  = 624 ,
  PawnValueMg    = 100  ,  PawnValueEg    = 135 ,
  KnightValueMg  = 550 ,  KnightValueEg  = 754 ,
  BishopValueMg  = 222 ,  BishopValueEg  = 220 ,
};
...
enum Direction : int {
  ...
  NORTH_EAST = NORTH + EAST,//象眼和士
  SOUTH_EAST = SOUTH + EAST,//象眼和士
  SOUTH_WEST = SOUTH + WEST,//象眼和士
  NORTH_WEST = NORTH + WEST//象眼和士
};
...
//棋子类型，使用与位操作，提高效率，相当于pc%7
constexpr PieceType type_of(Piece pc) {
  return PieceType(pc & 7);
}
//获取颜色，使用位与替换除法相当于除以2的三次方
...
//根据move计算开始Squere，利用位操作提高计算效率
constexpr Square from_sq(Move m) {
  return Square(m >> 7);
}
//根据move计算目的Squere，利用位操作提高计算效率
